...
class MedicalWorkflowAgent:
    ...
    def run(self, question: str) -> str:
        reasoning = self.call_llm(
            f"Given the medical context, plan to answer: {question}"
        )
...
def run_pipeline(
    ...
    debugged_workflow = tool_debugger.attach(
        workflow_agent, agent_id="medical_workflow"
    )

    # Add interactive breakpoints to exercise console commands
    tool_debugger.add_breakpoint(Breakpoint(breakpoint_type=BreakpointType.BEFORE_TOOL))
    tool_debugger.add_breakpoint(
        Breakpoint(breakpoint_type=BreakpointType.BEFORE_TOOL, tool_name="web_search")
    )
    tool_debugger.add_breakpoint(
        Breakpoint(
    ...
    tool_debugger.add_breakpoint(
        Breakpoint(breakpoint_type=BreakpointType.BEFORE_MEMORY_WRITE)
    )
    ...
    # Optional mocking
    if enable_mock:
        tool_debugger.mock_registry.mock_tool(
            "web_search", "Mocked search results from tool"
        )
        tool_debugger.mock_registry.mock_llm(
            "plan to answer", "Mocked LLM plan response"
        )
    ...
    if HAS_LANGCHAIN and reviewer_chain is not None:
        ...
        debugged_reviewer = review_debugger.attach(
            reviewer_chain, agent_id="reviewer_agent"
        )
        review_debugger.add_breakpoint(
            Breakpoint(breakpoint_type=BreakpointType.AFTER_REASONING)
        )
        review_debugger.add_breakpoint(
            Breakpoint(breakpoint_type=BreakpointType.BEFORE_REASONING)
        )
    ...
    if HAS_LANGCHAIN and reviewer_chain is not None:
        ...
        reviewed_output = (
            f"Patient-friendly: {medical_answer[:120]}\nConfidence: Medium"
        )
    ...
    if enable_multi and HAS_LANGCHAIN:
        ...
        if review_debugger is not None:
            review_debugger.add_breakpoint(
                Breakpoint(
                    breakpoint_type=BreakpointType.CONDITIONAL,
                    condition=lambda e: getattr(e, "event_type", None)
                    and e.event_type.value == "inter_agent_message",
                )
            )

        msg1 = orchestrator.send(
            "agent_A", "agent_B", "Propose a project name for a medical note app."
        )
...
def main():
    ...
    medical_answer, reviewed_output, dbg_tools, dbg_review, evt_counts, orch = (
        run_pipeline(
            question,
