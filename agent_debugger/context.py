...
class ContextManager:
    ...
    def __init__(self, max_context_items: int = 10000, auto_cleanup: bool = True):
        ...
        self.type_index: Dict[ContextType, Set[str]] = {}  # type -> set of context item IDs
        self.agent_index: Dict[str, Set[str]] = {}  # agent_id -> set of context item IDs
    ...
    def get_contexts_by_key(self, key: str) -> List[ContextItem]:
        ...
        return [self.context_items[cid] for cid in context_ids if cid in self.context_items]
    ...
    def search_contexts(self, 
                       ...
                       priority_filter: Optional[List[ContextPriority]] = None) -> List[ContextItem]:
    ...
    def get_context_summary(self) -> Dict[str, Any]:
        ...
        for context_item in self.context_items.values():
            ...
            if context_item.agent_id:
                summary['by_agent'][context_item.agent_id] = summary['by_agent'].get(context_item.agent_id, 0) + 1
        ...
        for context_item in self.context_items.values():
            ...
            if filters:
                if 'type' in filters and context_item.type != filters['type']:
                    continue
                if 'agent_id' in filters and context_item.agent_id != filters['agent_id']:
                    continue
                if 'tags' in filters and not any(tag in context_item.tags for tag in filters['tags']):
                    continue
            ...
            for context_data in contexts_data:
                try:
                    ...
                    if context_data.get('expires_at'):
                        context_data['expires_at'] = datetime.fromisoformat(context_data['expires_at'])
    ...
    def _cleanup_old_items(self):
        ...
        expired_items = [cid for cid, item in self.context_items.items() if item.is_expired()]
        ...
        if len(self.context_items) > self.max_context_items:
            items_by_priority = []
            for priority in [ContextPriority.ARCHIVE, ContextPriority.LOW, ContextPriority.MEDIUM, ContextPriority.HIGH]:
                for cid in self.priority_queues[priority]:
                    if cid in self.context_items:
                        items_by_priority.append((cid, self.context_items[cid].created_at))
