...
def auto_detect_framework(agent: Any) -> Optional[str]:
    ...
    # LangChain detection
    if (
        hasattr(agent, "invoke") and hasattr(agent, "ainvoke")
    ) or "chain" in agent_class_name:
        return "langchain"
    # CrewAI detection
    elif (
        hasattr(agent, "role") and hasattr(agent, "goal")
    ) or "crew" in agent_class_name:
        return "crewai"
    # AutoGPT detection
    elif (
        hasattr(agent, "ai_name") and hasattr(agent, "execute_command")
    ) or "autogpt" in agent_class_name:
        return "autogpt"
    # LlamaIndex detection
    elif (
        hasattr(agent, "query") and hasattr(agent, "retrieve")
    ) or "index" in agent_class_name:
        return "llamaindex"
    # OpenAI detection
