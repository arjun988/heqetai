            ...
            async def ainvoke(self, input_data, config=None, **kwargs):
                if self._debug_ainvoke is None:
                    ...
                    return await asyncio.get_event_loop().run_in_executor(
                        None, self.invoke, input_data, config, **kwargs
                    )
        ...
        if hasattr(agent, "run"):
            ...
            def debugged_run(input_text, **kwargs):
                ...
                end_event = TraceEvent(
                    ...
                    agent_id=agent_id,
                    step_id=event.step_id,
                    data={"output": result, "duration": duration},
                    parent_event_id=event.id,
                )
                self.debugger.trace_events.append(end_event)
    ...
    def _instrument_tools(self, tools: List[Any], agent_id: str):
        ...
        for i, tool in enumerate(tools):
            ...
            if hasattr(tool, "_run"):
                self._instrument_tool_method(
                    tool, "_run", tool_name, agent_id, sync=True
                )
            if hasattr(tool, "_arun"):
                self._instrument_tool_method(
                    tool, "_arun", tool_name, agent_id, sync=False
                )

    def _instrument_tool_method(
        self,
        tool: Any,
        method_name: str,
        tool_name: str,
