    ...
    def _instrument_async_query_engine(self, engine: Any, agent_id: str):
        ...
        async def debugged_aquery(query_str, **kwargs):
            ...
            end_event = TraceEvent(
                ...
                data={
                    "response": (
                        str(result.response)[:200] + "..."
                        if hasattr(result, "response")
                        and len(str(result.response)) > 200
                        else (
                            str(result.response)
                            if hasattr(result, "response")
                            else str(result)[:200]
        ...
        def debugged_retrieve(query_str, **kwargs):
            ...
            end_event = TraceEvent(
                ...
                data={
                    ...
                    "top_score": (
                        nodes[0].score if nodes and hasattr(nodes[0], "score") else None
                    ),
                    "duration": duration,
                },
                parent_event_id=event.id,
...
class LlamaIndexDebugger(AgentDebugger):
    ...
    def attach_async(self, agent_or_engine, agent_id: Optional[str] = None):
        ...
        debugged_entity = self.integration.instrument_async_agent(
            agent_or_engine, agent_id
        )
