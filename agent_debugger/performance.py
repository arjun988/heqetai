...
class PerformanceMonitor:
    ...
    def __init__(self, max_history: int = 1000):
        ...
        self.agent_metrics: Dict[str, Dict[str, deque]] = defaultdict(lambda: {
    ...
    def end_timing(self, event_id: str) -> Optional[float]:
        ...
        if event_id in self.start_times:
            ...
            if timing_info['agent_id']:
                self.record_agent_metric(timing_info['agent_id'], timing_info['event_type'], duration)
    ...
    def update_system_metrics(self):
        ...
        try:
            ...
            if disk_io:
                self.record_system_metric('disk_io', disk_io.read_bytes + disk_io.write_bytes)
            
            # Network I/O
            network_io = psutil.net_io_counters()
            if network_io:
                self.record_system_metric('network_io', network_io.bytes_sent + network_io.bytes_recv)
    ...
    def calculate_efficiency_score(self, agent_id: str) -> float:
        ...
        task_completion_rate = len(agent_data['task_completion_times']) / max(1, len(agent_data['error_count']))
        error_rate = sum(agent_data['error_count']) / max(1, len(agent_data['error_count']))
        avg_response_time = sum(agent_data['task_completion_times']) / max(1, len(agent_data['task_completion_times']))
        
        # Efficiency score (0-100)
        efficiency = (task_completion_rate * 40) + ((1 - error_rate) * 30) + (max(0, 10 - avg_response_time) * 3)
    ...
    def _get_metric_stats(self, metrics_dict: Dict[str, deque]) -> Dict[str, Dict[str, float]]:
        ...
        for metric_type, values in metrics_dict.items():
            if values:
                ...
                stats[metric_type] = {'count': 0, 'mean': 0, 'max': 0, 'min': 0, 'latest': 0, 'median': 0}
        return stats
    ...
    def _check_performance_alerts(self, metric_type: str, value: float):
        ...
        if metric_type == 'cpu_usage' and value > self.alert_thresholds['high_cpu_usage']:
            alert_triggered = True
            alert_message = f"High CPU usage: {value:.1f}%"
        elif metric_type == 'memory_usage' and value > self.alert_thresholds['high_memory_usage']:
            alert_triggered = True
            alert_message = f"High memory usage: {value:.1f}%"
        elif metric_type in ['tool_execution_times', 'llm_response_times'] and value > self.alert_thresholds['slow_response_time']:
            alert_triggered = True
            alert_message = f"Slow {metric_type}: {value:.2f}s"
        elif metric_type == 'error_rates' and value > self.alert_thresholds['high_error_rate']:
            alert_triggered = True
        ...
        if alert_triggered:
            self.alerts.append({
                ...
                'severity': 'warning' if value < self.alert_thresholds.get(metric_type, 0) * 1.5 else 'critical'
            })

    def get_chart_data(self, metric_type: str, time_range_minutes: int = 60) -> Dict[str, Any]:
        """Get data formatted for charts"""
        cutoff_time = datetime.now() - timedelta(minutes=time_range_minutes)
        
        if metric_type in self.metrics:
            values = list(self.metrics[metric_type])
            timestamps = [self.session_start + timedelta(seconds=i*10) for i in range(len(values))]
