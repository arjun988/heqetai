...
class DebugConsole:
    ...
    def _list_breakpoints(self, event: TraceEvent, state: AgentState):
        ...
        for bp in self.debugger.breakpoints:
            ...
            print(
                f"  {status} {bp.id}: {bp.breakpoint_type.value} (hits: {bp.hit_count})"
            )
    ...
    def _show_trace(self, event: TraceEvent, state: AgentState):
        ...
        for trace_event in self.debugger.trace_events[-10:]:
            ...
            print(
                f"  [{trace_event.timestamp.strftime('%H:%M:%S')}{duration}] {trace_event.agent_id} - {trace_event.event_type.value}"
            )
    ...
    def _performance(self, event: TraceEvent, state: AgentState):
        ...
        for metric, values in stats.items():
            if values["count"] > 0:
                print(
                    f"  {metric}: {values['mean']:.3f}s avg ({values['count']} samples)"
                )

    def _watch_variable(
        self, event: TraceEvent, state: AgentState, variable_name: str = ""
    ):
        ...
        def watch_condition(evt: TraceEvent) -> bool:
            return (
                evt.event_type == EventType.MEMORY_WRITE
                and evt.data.get("key") == variable_name
            )
    ...
    def _list_agents(self, event: TraceEvent, state: AgentState):
        ...
        for agent_id, agent_state in self.debugger.agent_states.items():
            ...
            print(
                f"  {status} {agent_id} (tasks: {agent_state.tasks_completed}, errors: {agent_state.errors_encountered})"
            )
  ...
  ctx summary - Show context summary
    ...
    def _add_context(self, event: TraceEvent, state: AgentState, args: str = ""):
        ...
        try:
            ...
            # Get context type
            print(
                "Available types: memory, conversation, knowledge, task, environment, user_preference, system_state"
            )
            ...
            agent_id = (
                input(f"Agent ID (optional, current: {event.agent_id}): ").strip()
                or event.agent_id
            )
            step_id = (
                input(f"Step ID (optional, current: {event.step_id}): ").strip()
                or event.step_id
            )
    ...
    def _search_contexts(self, event: TraceEvent, state: AgentState, query: str = ""):
        ...
        try:
            ...
            for ctx in contexts[:10]:  # Show first 10 results
                ...
                print(
                    f"    Value: {str(ctx.value)[:100]}{'...' if len(str(ctx.value)) > 100 else ''}"
                )
    ...
    def _list_contexts(self, event: TraceEvent, state: AgentState):
        ...
        try:
            ...
            for ctx in contexts:
                ...
                print(f"    Agent: {ctx.agent_id}, Created: {ctx.created_at}")
                print(
                    f"    Value: {str(ctx.value)[:50]}{'...' if len(str(ctx.value)) > 50 else ''}"
                )
                if ctx.tags:
    ...
    def _clear_contexts(self, event: TraceEvent, state: AgentState, args: str = ""):
        ...
        try:
            ...
            tags = (
                [tag.strip() for tag in tag_filter.split(",")] if tag_filter else None
            )

            if not type_filter and not agent_filter and not tag_filter:
                confirm = (
                    input("Clear ALL contexts? This cannot be undone! (yes/no): ")
                    .strip()
                    .lower()
                )
                if confirm != "yes":
            ...
            cleared_count = self.debugger.clear_contexts(
                type_filter=type_enum, agent_filter=agent_filter, tag_filter=tags
            )
    ...
    def _export_contexts(self, event: TraceEvent, state: AgentState):
        ...
        try:
            ...
            if not filename:
                filename = (
                    f"contexts_export_{event.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
                )
